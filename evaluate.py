...
def eval_clipscore(args):
    image_list = [os.path.join(args.gen_img_dir, f) for f in os.listdir(args.gen_img_dir) if f.endswith('.png') or f.endswith('.jpg')]
    image_list.sort()
...
def main(args):
    if args.eval_fid: 
        assert args.gt_img_dir is not None, "Please provide path to ground truth image directory"
        eval_fid(args)
    elif args.eval_clipscore:
        assert args.prompt_txt_path is not None, "Please provide path to prompt text file"
        eval_clipscore(args)
