...
if __name__ == '__main__':
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    #gmm = TwoPointGMM(x=10.0, y=10.0, std=1.0, device=device)
    gmm = CircularGMM(n_components=6, radius=10, dim=2, std=1.0, device=device)
    x = gmm.sample_with_labels([1000])
    x, labels = gmm.sample_with_labels([1000])
    plt.plot(x[:,0].cpu(), x[:,1].cpu(),'.')
    plt.plot(x[labels==0,0].cpu(), x[labels==0,1].cpu(),'.r')
