...
def get_clip_score(model, images_feats, candidates, device, w=2.5):
    ...
    if version.parse(np.__version__) < version.parse('1.21'):
        ...
        images_feats = images_feats / np.sqrt(np.sum(images_feats**2, axis=1, keepdims=True))
...
def calculate_clipscore(
    ...
    image_feats = extract_feats(image_list, model, device, batch_size=batch_size, num_workers=num_workers)
    
    # get image-text clipscore
    _, per_instance_image_text, candidate_feats = get_clip_score(model, image_feats, text_list, device, w=1.)
