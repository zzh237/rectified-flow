...
def main(args):
    ...
    if args.model_name == "unet": model_cls = SongUNet
    elif args.model_name == "dit": model_cls = DiT
    ...
    with torch.inference_mode():
        for iteration in range(args.iterations):
            print(f"Iteration: {iteration}, images from {iteration * args.batch_size} to {(iteration + 1) * args.batch_size}")
            z_0 = torch.randn(args.batch_size, 3, 32, 32, device=device)
            z_1 = rk45(model, z_0, startT=0., endT=1.0)
            z_1 = (z_1 * 0.5 + 0.5).clamp(0, 1)
            for i in range(z_1.shape[0]):
                save_image(z_1[i], os.path.join(args.output_dir, f"{iteration}_{i}.png"))

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run ODE-based image generation using DiT.")
    parser.add_argument("--model_name", type=str, default="unet", help="Name of the model to use.")
    parser.add_argument("--ckpt_path", type=str, default="/root/autodl-tmp/unet_cifar/checkpoint-500000", help="Path to the checkpoint file.")
    parser.add_argument("--output_dir", type=str, default="/root/autodl-tmp/unet_cifar/checkpoint-500000/samples", help="Directory to save generated images.")
